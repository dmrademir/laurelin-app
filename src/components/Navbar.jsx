...
export default function Navbar() {
  ...
  return (
    <header
      className={`header-main-v5 ${showNavbar ? "visible" : "hidden-scroll"}`}
    >
      {/* NÍVEL 1: LOGO CENTRALIZADO */}
      <div
        className="logo-row-v5"
        onClick={() => {
          navigate("/");
          closeMenu();
        }}
      ...
      <nav className={`nav-row-v5 ${showNavbar ? "visible" : "hidden-scroll"}`}>
        <div className="nav-links-container-v5">
          ...
          <Link to="/curiosidades" onClick={closeMenu}>
            Curiosidades
          </Link>
          <Link to="/estilos" onClick={closeMenu}>
            Estilos
          </Link>
          <Link to="/produto" onClick={closeMenu}>
            Produto
          </Link>
          <Link to="/harmonizacoes" onClick={closeMenu}>
            Harmonizações
          </Link>
          ...
          <a
            href="https://produto.mercadolivre.com.br/MLB-6257299332"
            target="_blank"
